...
int main(int argn, char* argv[])
{
	...
	const float border_strenth = 50.f;
	...
	for (int i = 0; i < stream_numbers; i = i + 1)
	{
		...
		VECTOR rand_vec = { (float)0.1f * rand() / RAND_MAX + 0.05f, (float)0.1f * rand() / RAND_MAX + 0.05f, (float)0.1f * rand() / RAND_MAX + 0.05f };
		//for (int j = 0; j < u_freedom - 6; j = j + 1)
		//{
		//	molecule[i].vina_gpu.h_u_crd[j] = 2.f * 3.141592654f * rand() / RAND_MAX;
		//}
		rand_max_val = 2.f * 3.141592654f * rand() / RAND_MAX;
		//printf("RAND_MAX is %f.\n", rand_max_val);
		//printf("rand_vec is %f, %f, %f.\n", rand_vec.x, rand_vec.y, rand_vec.z);
		...
		molecule[i].vina_gpu.last_accepted_energy = 500000000.f;
		...
		for (int i = 0; i < stream_numbers; i = i + 1)
		{
			if (task[i].Is_empty())//如果当前stream无任务
			{
				if (task[i].Get_Status() == DSDP_TASK_STATUS::MINIMIZE_STRUCTURE)//如果当前stream是做完了一次最优化
				{
					...
					//printf("molecule %d energy is %f.\n", i, current_energy);
					// intra energy
					float current_intra_energy = molecule[i].vina_gpu.h_u_crd[u_freedom + 1];
					//printf("molecule %d intra energy is %f.\n", i, current_intra_energy);
				...
				if (rand_int < u_freedom - 3)
				{
					if (rand_int < u_freedom - 6)
					{
						...
						molecule[i].vina_gpu.h_u_crd[rand_int] = 0.005f * 3.141592654f * ((float)rand() / RAND_MAX);
					}
					else
					{
						//molecule[i].vina_gpu.h_u_crd[rand_int] += 1.f * (2.f * ((float)rand() / RAND_MAX) - 1.f);
						molecule[i].vina_gpu.h_u_crd[rand_int] += 0.f;
					}
				}
				else
				{
				...
				//printf("molecule %d force is %f, %f, %f.\n", i, molecule[i].vina_gpu.u_crd, molecule[i].vina_gpu.last_u_crd, molecule[i].vina_gpu.dU_du_crd, molecule[i].vina_gpu.last_dU_du_crd);
				Optimize_Structure_BB2_Direct_Pair_Device << <1, 128, sizeof(float) * 24, task[i].Get_Stream() >> > 
					(
						...
						vgff.grid.texObj_for_kernel, border_strenth ,
	...
	DSDP_sort.Sort_Structures(
		...
		//-0.5f, 20, 20);
	printf("Search 2 is started, energy_record is %d, selected number is %d.\n", (int)energy_record.size(), DSDP_sort.selected_numbers);
	...
	DSDP_sort.Sort_Structures(
		...
		rmsd_similarity_cutoff, 20, 20);
		//-0.5f, 20, 20);
	printf("Search 2 is done, energy_record is %d, selected number is %d.\n", (int)energy_record.size(), DSDP_sort.selected_numbers);
	//out
