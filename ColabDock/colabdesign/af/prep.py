...
class _af_prep:
  ...
  def _prep_dock(self, rest_set, template, fixed_chains=None, use_initial=True, msas=None,
    ...
    #print(pdb['batch']['all_atom_positions'])
    #print(pdb['all_atom_positions'])
    ...
    print('Fixed chains self.cliques:')
    print(self.cliques)
    ...
    print('initial init_mask:')
    print(init_mask)
    noise_mask = np.random.rand(self._len)
    print('initial noise_mask:')
    print(noise_mask)
    noise_mask = np.where(noise_mask > 0.7, 1, 0)
    init_mask += noise_mask*(1. - init_mask)
    print('binary noise_mask:')
    print(noise_mask)
    print('init_mask:')
    print(init_mask)

    #fig, ax = plt.subplots(2, 1, figsize=(200, 20))

    seq_init = np.eye(20)[self._wt_aatype] * init_mask[..., None]
    print('seq_init 0:')
    print(seq_init)
    tem = seq_init
    #im0 = ax[0].imshow(np.transpose(tem), origin='upper')
    seq_init += np.random.normal(size=(self._len, 20)) * 0.1 * (1. - init_mask[..., None])
    print('seq_init 1:')
    print(seq_init)
    #im1 = ax[1].imshow(np.transpose(seq_init), origin='upper')

    # add residue ID labels to axes
    #tick_interval = 20
    #plt.title('template_dist')
    # colorbar
    #cbar0 = fig.colorbar(im0, ax=ax[0])
    #cbar0.ax.set_ylabel('seq_init 0')
    #cbar1 = fig.colorbar(im1, ax=ax[1])
    #cbar1.ax.set_ylabel('seq_init 1')
    #plt.savefig('seq_init.png')
    ...
    if use_initial:
      ...
      mat_rot = [R.from_euler('zyx', [0, 0, 0], degrees=True) for _ in range(len(cliques))]
      ...
      for ind, iclique in enumerate(cliques):
        for ichain in iclique:
          ...
          ipdb_pos = mat_rot[0].apply(ipdb_pos.reshape([-1, 3])).reshape([self.lens[ichain], 37, 3])
      ...
      print('coord after random rotation:')
      print(coord.shape)
      print(coord)
      #print('pdb_max after random rotation:')
      #print(pdb_max)
      #print('pdb_min after random rotation:')
      #print(pdb_min)
    ...
    #im0 = ax[0].imshow(dm, origin='upper')
    #im1 = ax[1].imshow(dm_mask, origin='upper')
    # add residue ID labels to axes
    #tick_interval = 20
    #plt.title('template_dist')
    # colorbar
    #cbar0 = fig.colorbar(im0, ax=ax[0])
    #cbar0.ax.set_ylabel('template_dist')
    #cbar1 = fig.colorbar(im1, ax=ax[1])
    #cbar1.ax.set_ylabel('template_dist_mask')
    #plt.savefig('template_dist.png')
    ...
    print("use_templates is:")
    print(self._args["use_templates"])
    ...
    #print(self._inputs)
    #np.savetxt('all_atom_positions.out', self._inputs['all_atom_positions'], delimiter=',')
...
def prep_pdb(pdb_filename, chain=None, for_alphafold=True):
  ...
  for chain in chains:
    ...
    # for_alphafold from ColabDock.setup is False
    # for_alphafold from _prep_dock is True
    if for_alphafold:
